...
import type { BaseConfig, ProfileConfig } from '../types/config.types.js';
...
interface FileNode {
  ...
  path: string;
  type: 'file' | 'directory';
  mtime?: number; // epoch ms, only for files
  children?: FileNode[];
}
...
function getRecentlyChangedFiles(sourcePath: string, commits: number = 5): string[] {
...
function buildFoldersFromSelection(
  ...
  const folders: (string | { [key: string]: string[] })[] = [];
...
export async function startServer(port: number, cfgPath?: string): Promise<number> {
  ...
  app.post('/api/profiles', (req, res) => {
    try {
      ...
      const { name, profile } = req.body as { name: string; profile: ProfileConfig };
  ...
  app.post('/api/profiles/:name/verify-password', (req, res) => {
    try {
      ...
      const profile = config.profiles[req.params.name];
      if (!profile) return res.status(404).json({ error: 'Profile not found' });
      if (!profile.password) return res.json({ valid: true });
    ...
    try {
      ...
      const { password, settings } = req.body as {
        ...
        settings: {
          ...
          stagingSSH?: { host: string; port: number; username: string; privateKey?: string };
          destinationSSH?: { host: string; port: number; username: string; privateKey?: string };
        };
      };
