...
export async function deployCommand(config, sourceFolders, looseFiles) {
            ...
            `.replace(/\n/g, ' ');
                        ...
                        proc.on('close', (code) => {
                            if (code === 0)
                                ...
                                reject(new Error(`update.bat exited with code ${code}: ${stderr || stdout}`));
                        });
                    });
                }
                else {
                    ...
                    if (!ssh)
                        throw new Error('SSH connection required to execute update.bat on staging');
