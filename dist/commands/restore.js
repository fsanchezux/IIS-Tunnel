...
export async function restoreCommand(config) {
          ...
          `.replace(/\n/g, ' ');
                    ...
                    proc.on('close', (code) => {
                        if (code === 0)
                            ...
                            reject(new Error(`restore.bat exited with code ${code}: ${stderr || stdout}`));
                    });
                });
            }
            else {
                ...
                if (!ssh)
                    throw new Error('SSH connection required to execute restore.bat on staging');
