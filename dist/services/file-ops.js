...
export class FileOpsService {
    ...
    async ensureSSHConnection(location) {
    ...
    async copyFiles(source, dest, folders) {
        ...
        if (source.type === 'local' && dest.type === 'ssh') {
            ...
            if (!ssh)
                throw new Error('SSH connection required for destination');
        ...
        if (source.type === 'ssh' && dest.type === 'local') {
            ...
            if (!ssh)
                throw new Error('SSH connection required for source');
    ...
    async copySpecificFolders(source, dest, folders) {
        let totalCount = 0;
        for (const folderSpec of folders) {
            if (typeof folderSpec === 'string') {
                ...
                for (const [folderName, files] of Object.entries(folderSpec)) {
                    ...
                    totalCount += await this.copySpecificFiles(source, dest, folderName, files);
                }
            }
        }
        return totalCount;
    }
    async copySpecificFiles(source, dest, folderName, files) {
        let count = 0;
        const srcFolderPath = source.type === 'local'
            ? path.join(source.path, folderName)
            : this.joinPath(source.path, folderName);
    ...
    async copyLocalToLocal(srcPath, destPath) {
    ...
    async copyLocalToSSH(srcPath, destPath, ssh) {
        // Verify source exists
        if (!(await fs.pathExists(srcPath))) {
            throw new Error(`Source path does not exist: ${srcPath}`);
        }
    ...
    async uploadDirectory(localPath, remotePath, ssh) {
    ...
    async copySSHToLocal(srcPath, destPath, ssh) {
    ...
    async downloadDirectory(remotePath, localPath, ssh) {
    ...
    async deleteLocalContents(dirPath) {
    ...
    async deleteSSHContents(dirPath, ssh) {
    ...
    async compressFiles(sourcePath, outputPath, onProgress) {
        if (onProgress)
            onProgress(5);
        try {
    ...
    compressWithPowerShell(sourcePath, outputPath) {
...
`;
    ...
    compressWithArchiver(sourcePath, outputPath) {
        return new Promise(async (resolve, reject) => {
            try {
    ...
    compressWith7Zip(sourcePath, outputPath) {
        return new Promise(async (resolve, reject) => {
            try {
                await fs.ensureDir(path.dirname(outputPath));
                ...
                try {
                    ...
                    const b = sevenBin;
                    sevenCmd = (b.path7za || b.path7x || b.path7z || sevenCmd);
                }
                catch (e) {
    ...
    async decompressFiles(zipPath, outputPath) {
        await extractZip(zipPath, { dir: outputPath });
    }
    async copyToTemp(source, staging, folders, tempDir, files) {
    ...
    async copyLooseFiles(source, tempDir, files) {
        let count = 0;
        for (const file of files) {
            const srcFile = source.type === 'local'
    ...
    async transferFile(localZipPath, stagingLocation, fileName, onProgress) {
        if (stagingLocation.type === 'local') {
            ...
            if (!ssh)
                throw new Error('SSH connection required for staging');
    ...
    async decompressFilesRemote(stagingLocation, zipFileName) {
        if (stagingLocation.type === 'local') {
    ...
    generateRestoreBat(staging, destination, backup) {
        const sourceDir = staging.path;
        const destDir = destination.path;
        const backupDir = backup.path;
    ...
    async deleteRestoreBat(staging) {
        if (staging.type === 'local') {
            const targetRestore = path.join(staging.path, 'restore.bat');
            try {
                await fs.remove(targetRestore);
            }
            catch { /* ignore */ }
        }
        else {
    ...
    isSameSSHServer(location1, location2) {
        ...
        return (location1.ssh.host === location2.ssh.host &&
            location1.ssh.port === location2.ssh.port &&
            location1.ssh.username === location2.ssh.username);
    }
    async copyFilesOnRemoteServer(stagingPath, destinationPath, ssh) {
    ...
    async canUseSameSSHConnection(staging, destination) {
